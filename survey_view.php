...
//must include query string: survey_view.php?id=1
//dumpDie($mySurvey);

if($mySurvey->IsValid)
{//good survey, show data
    echo '<h3 align="center">' . $mySurvey->Title . '</h3>';
}
else
{//apologize
...
class Survey
{
    ...
    public function __construct($id)
    {
        ...
        {#there are records - present data
            ...
            while($row = mysqli_fetch_assoc($result))
            {# pull data from associative array
                $this->Questions[] = new Question($row['QuestionID'], $row['Question']); //Question objects are created from Survey constructor
                //$this->Title = $row['Title'];
                //$this->Description = $row['Description'];
            }
        }
